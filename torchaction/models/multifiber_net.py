...
class MultiFiberNet3d(nn.Module):
    ...
    def __init__(self, num_classes):
        """
        Arguments
            .. num_classes (int): number of classification group
        """
...
class MultiFiberUnit(nn.Module):
    ...
    self.conv_m1 = _conv_fiber_unit(in_ch, mux_ch, kernel_size=(3,3,3), padding=(1,1,1), stride=main_stride, groups=16)

    if is_first:
        self.conv_m2 = _conv_fiber_unit(mux_ch, out_ch, kernel_size=(1,1,1), padding=(0,0,0), stride=(1,1,1), groups=1)
        self.conv_w1 = _conv_fiber_unit(in_ch, out_ch, kernel_size=(1,1,1), padding=(0,0,0), stride=main_stride, groups=1)
    else:
